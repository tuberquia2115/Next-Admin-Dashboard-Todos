...
const postSchema = yup.object({
  ...
  complete: yup.bool().optional().default(false), //! TODO: mostrar algo interesante
...
export async function POST(request: Request) {
  ...
  try {
    const { description, complete, title } = await postSchema.validate(await request.json());

    const todo = await prisma.todo.create({ data: { complete, description, title, userId: user?.id } });
